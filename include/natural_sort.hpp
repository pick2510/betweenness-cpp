...
template <typename ElementType> bool is_not_digit(const ElementType &x)
{
  return !isdigit(x);
}

/********** Compare Two Iterators CaseInsensitive ********/
template <typename ElementType, typename Iterator> struct comp_over_iterator {
...
template <typename ValueType, typename Iterator> struct compare_number {
private:
  // If Number is Itself fractional Part
  int fractional(Iterator lhsBegin, Iterator lhsEnd, Iterator rhsBegin,
                 Iterator rhsEnd)
  {
    while (lhsBegin < lhsEnd && rhsBegin < rhsEnd) {
      int local_compare =
          comp_over_iterator<ValueType, Iterator>()(lhsBegin, rhsBegin);
      if (local_compare != 0)
  ...
  int non_fractional(Iterator lhsBegin, Iterator lhsEnd, Iterator rhsBegin,
                     Iterator rhsEnd)
  {
    // Skip Inital Zero's
    while (lhsBegin < lhsEnd && *lhsBegin == '0')
...
bool _compare(const Iterator &lhsBegin, const Iterator &lhsEnd,
    ...
    if (!isdigit(*current1) || !isdigit(*current2)) {
      ...
      ***********************************/
      Iterator last_nondigit1 =
          std::find_if(current1, lhsEnd, detail::is_not_digit<ElementType>);
      Iterator last_nondigit2 =
          std::find_if(current2, rhsEnd, detail::is_not_digit<ElementType>);

      int result = detail::compare_number<ElementType, Iterator>()(
          current1, last_nondigit1,
          (current1 > lhsBegin && *(current1 - 1) == '.'), current2,
          last_nondigit2, (current2 > rhsBegin && *(current2 - 1) == '.'));
      if (result < 0)
        return true;
      if (result > 0)
        return false;
...
template <> inline bool compare(char *const &first, char *const &second)
{
  char *it1 = first;
  while (*it1 != '\0')
    it1++;
...
template <typename Container> inline void sort(Container &container)
{
  std::sort(container.begin(), container.end(),
            compare<typename Container::value_type>);
}
...
template <typename ValueType, int N> inline void sort(ValueType container[N])
{
